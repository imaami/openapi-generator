...
static size_t writeDataCallback(char *buffer, size_t size, size_t nmemb, void *userp);
...
static size_t writeDataCallback(char *buffer, size_t size, size_t nmemb, void *userp) {
    ...
    size_t len = apiClient->dataReceivedLen >= 0
                 ? (size_t)apiClient->dataReceivedLen
                 : 0U;
    char *data = realloc(apiClient->dataReceived,
                         len + size_this_time + 1);
    if (!data) {
        return 0;
    }
    memcpy(&data[len], buffer, size_this_time);
    data[len] = '\0';

    apiClient->dataReceived = data;
    apiClient->dataReceivedLen = (long)(len + size_this_time);
